...
- Giữ gating link/đường dẫn tài liệu theo gia tăng qua `./scripts/ci/docs_links_gate.sh` (chặn repo-relative link hỏng mới thêm, đường dẫn workspace tuyệt đối và ví dụ đường dẫn người dùng không theo XDG trên dòng thay đổi); giữ audit URL ngoài là tùy chọn qua `./scripts/ci/docs_external_links_audit.sh`.
